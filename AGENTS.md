...
- **Km coherence:** all km-based metrics (km chart, maintenance projection, group analysis, `reporte-vehiculo.js` km traveled) discard typo readings via `filterKmReadings()`; discarded counts are surfaced as notes. The form (`app.js`) never blocks saving: km lower than the vehicle's last reliable Supabase reading or a jump over `KM_MAX_JUMP` triggers a confirm (`confirmKmSospechoso`); on accept the report is saved with `km_sospechoso = true` and `km_nota` (i18n keys `valKmRetrocede` / `valKmSalto`, last reading via `getLastKmFromSupabase()`, which skips flagged rows). `filterKmReadings(list, {includeFlagged})` excludes flagged readings by default and returns `{valid, discarded, flagged, excluded}`; the quincenal page has a "Incluir kilometrajes marcados como dudosos" checkbox (`#q-incluir-dudosos`) to bring them back in. Columns come from `migration_km_sospechoso.sql`; `saveReport()` retries without them if the migration hasn't been applied.
